			...
			while (true) {
				if (!steep) {
					if (0 <= _y && _y < width && 0 <= _x && _x < height) {
						if (map.isOccupied(_y, _x)) {  // Check if the current cell is occupied
							float xd = _x - x0;
							float yd = _y - y0;
							// printf("case 1: in bounds, occupied: %f, %f \n", _x, _y);
							return sqrtf(xd*xd + yd*yd);
						}
					} else {  // Out of bounds, calculate distance to the last in-bound point
						float xd = _x - xstep - x0;  // Subtract xstep because _x has already moved out of bounds
						float yd = _y - ystep - y0;
						// printf("case 2: out of bounds: %f, %f \n", _x, _y);
						return sqrtf(xd*xd + yd*yd);
					}
				} else {
					if (0 <= _x && _x < width && 0 <= _y && _y < height) {
						if (map.isOccupied(_x, _y)) {  // Check if the current cell is occupied
							float xd = _x - x0;
							float yd = _y - y0;
							// printf("case 3: in bounds, occupied: %f, %f \n", _x, _y);
							return sqrtf(xd*xd + yd*yd);
						}
					} else {  // Out of bounds, calculate distance to the last in-bound point
						float xd = _x - xstep - x0;
						float yd = _y - ystep - y0;  // Subtract ystep because _y has already moved out of bounds
						// printf("case 4: out of bounds: %f, %f \n", _x, _y);
						return sqrtf(xd*xd + yd*yd);
					}
				}
				...
				// // Break if the next point is beyond the endpoint
    			if ((xstep > 0 && _x > x1) || (xstep < 0 && _x < x1)) {
					// printf("case 6: beyond enpoint \n");
					break;
				}
			}
			// printf("case 5: reached end of loop \n");
			return max_range; 
		}
